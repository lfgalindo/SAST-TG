...
public class TreatDate {
	...
	/**
	 * 
	 * Método responsável por verificar se a data é futura
	 * @since 27/11/2015 18:11:16
	 * @author Yan Felipe de Souza <yan.felipe@sigma.com.br>(Projeto)
	 * @since 27/11/2015 18:11:16
	 * @author Yan Felipe de Souza <yan.felipe@sigma.com.br> (Implementação)
	 * @param data
	 * @return 
	 */
	
	public static boolean isFuturo(Date data){
		return data.after(Calendar.getInstance().getTime());
	}
	...
	public static String concatenarPeriodo(Date dataInicial, Date dataFinal){
		...
		periodo.append(TreatDate.format("dd/MM/yyyy", dataInicial));
		if(Util.isNotNull(dataFinal)){
			periodo.append(TreatDate.format(" 'a' dd/MM/yyyy", dataFinal));
		}
	 ...
	 * @return
	 */
	public static final Date adicionarMinutos(Date date, int minutos) {
		if (date == null) {
			return null;
		}
		Calendar cal = Calendar.getInstance();
